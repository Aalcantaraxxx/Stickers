            ...
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    await exec(`gify ${filename} ./media/stickergf.gif --fps=30 --scale=240:240`, async function (error, stdout, stderr) {
                        ...
                        .catch(() => {
                            aruga.reply(from, 'Lo sentimos, el archivo es demasiado grande. Por favor baja la calidad de tu video y vuelve a mandarlo, gracias.', id)
                        })
                    })
                  } else {
                    aruga.reply(from, `[❗] ERROR Envía nuevamente el gif *${prefix}stickergif* max 10 sec!`, id)
                   }
                } else {
